<?php
    ...
    //column names
    $fields = array('TRANSACTION_ID','TRANSACTION_DATE','SERVICE_NAME','TRANSACTION_AMOUNT','ORIG_USER_ID','ORIG_MSISDN','ORIG_FULL_NAME','DEST_MSISDN','DEST_FULL_NAME');
    ...
    if ($userId ==  5903 || 1638) { //bumthang MASTER AGENT TO ACCOUNTANT/ CUSTOMER FOR TOPUP
        $sql=" SELECT a.TRANSACTION_ID, a.TRANSACTION_DATE,a.SERVICE_NAME, a.TRANSACTION_AMOUNT,
        a.ORIG_USER_ID,  a.ORIG_MSISDN, a.ORIG_FULL_NAME,  
        a.DEST_MSISDN, a.DEST_FULL_NAME, a.ORIG_BAL_BEFORE, a.ORIG_BAL_AFTER,
        b.PARENT_ID, b. PARENT_NAME FROM MFS_CDR_MASTER a
        LEFT JOIN MFS_USER_VIEW b
        ON a.DEST_USER_ID=b.USER_ID WHERE 
        a. DEST_USER_ID IN (5229,5638,6919,5739) AND 
        ...
        if (isset($_GET['search']) && $_GET['search'] != null && isset($_GET['from_date']) && $_GET['from_date'] != null && isset($_GET['to_date']) && $_GET['to_date'] != null) {
            ...
            ON a.DEST_USER_ID=b.USER_ID WHERE 
            a.DEST_USER_ID IN (5229,5638,6919,5739) AND 
            (a.TRANSACTION_ID='$searchValue' OR a.ORIG_USER_ID='$searchValue' OR a.ORIG_MSISDN='$searchValue' OR a.DEST_MSISDN='$searchValue' OR b.PARENT_ID='$searchValue' OR b. PARENT_NAME='$searchValue' OR a.ORIG_FULL_NAME='$searchValue')AND 
            SERVICE_ID = '7' AND STATUS_TYPE = 'S' 
